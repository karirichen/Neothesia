...
pub fn deframe(x: &Array3<f32>) -> Array2<f32> {
...
pub fn get_binarized_output_from_regression(
...
pub fn is_monotonic_neighbour(x: &ArrayView1<f32>, n: usize, neighbour: usize) -> bool {
...
pub fn note_detection_with_onset_offset_regress(
    ...
    for piano_note in 0..classes_num {
        ...
        for (bgn, fin, bgn_shift, fin_shift) in res {
            let onset_time = (bgn as f32 + bgn_shift) / FRAMES_PER_SECOND as f32;
            let offset_time = (fin as f32 + fin_shift) / FRAMES_PER_SECOND as f32;
...
pub fn create_midi_file(notes: Vec<(usize, f32, f32)>) -> midly::Smf<'static> {
...
mod tests {
    ...
    fn binarization_flags_threshold_peak() {
        ...
        for (i, v) in [0.0_f32, 0.5, 0.9, 0.5, 0.0].into_iter().enumerate() {
            data[[i, 0]] = v;
        }
